...
api_key = "sorry we cant make it public"
...
def create_usertable():
    c.execute('CREATE TABLE IF NOT EXISTS userstable(username TEXT,password TEXT,phonenumber TEXT,alternateNo TEXT)')


def add_userdata(username,password,phonenumber,alternateNo):
    c.execute('INSERT INTO userstable(username,password,phonenumber,alternateNo) VALUES (?,?,?,?)',(username,password,phonenumber,alternateNo))
    conn.commit()
...
def create_usertable2():
    c2.execute('CREATE TABLE IF NOT EXISTS autotable(username TEXT,password TEXT,phoneNo TEXT,aadharNo TEXT,autoNumber TEXT)')


def fetchPhoneNumber():
    c2.execute('SELECT username,phoneNo,autoNumber from autotable')
    data = c2.fetchall()
    return data

def add_userdata2(username,password,phoneNo,aadharNo,autoNumber):
    c2.execute('INSERT INTO autotable(username,password,phoneNo,aadharNo,autoNumber) VALUES (?,?,?,?,?)',(username,password,phoneNo,aadharNo,autoNumber))
    conn2.commit()
    ...
    if choice == "Home":
        ...
        if st.sidebar.checkbox("Login"):
            ...
            if result:
                ...
                task = st.selectbox("Task",["Get Nearby Hubs","Go Online","Reserve Auto","Get Path"])
                if task == "Get Nearby Hubs":
                    ...
                    if btn:
                        ...
                        st.dataframe(pd.DataFrame(data,columns=["driver","phone","autoNumber"]))
        ...
        new_ph = st.text_input("Your phone number")
        new_ph2 = st.text_input("Alternate phone number")
        ...
        if st.button("Signup"):
            ...
            create_usertable()
            add_userdata(new_user,make_hashes(new_password),new_ph,new_ph2)
         ...
         new_aadharn = st.text_input("Aadhar number")
         new_auton = st.text_input("Auto number")
         if st.button("Signup"):
             ...
             create_usertable2()
             add_userdata2(new_user,make_hashes(new_password),new_phoneNo,new_aadharn,new_auton)
             st.success("You have successfully created a valid Account")
             st.info("Go to Login Menu to login")
main()